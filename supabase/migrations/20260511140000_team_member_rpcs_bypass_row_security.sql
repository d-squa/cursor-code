-- SECURITY DEFINER functions still evaluate RLS as the invoker unless row_security is disabled for the function.
-- Without this, DELETE/UPDATE on user_roles can silently affect 0 rows while auth checks inside the function pass.
...
COMMENT ON FUNCTION public.remove_team_member_from_team(uuid, uuid) IS
  'Deletes user_roles for one team if caller owns or manages that team; cancels matching pending invites. Uses row_security=off so RLS cannot block the delete.';

COMMENT ON FUNCTION public.update_team_member_role(uuid, uuid, public.app_role) IS
  'Updates user_roles.role for one team when caller owns or manages that team. Uses row_security=off so RLS cannot block the update.';
