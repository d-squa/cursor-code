...
-- Create setup_mistakes table
CREATE TABLE public.setup_mistakes (
...
CREATE INDEX idx_setup_mistakes_campaign ON public.setup_mistakes(campaign_id);
CREATE INDEX idx_setup_mistakes_qc_tracking ON public.setup_mistakes(qc_tracking_id);
CREATE INDEX idx_setup_mistakes_team ON public.setup_mistakes(team_id);
CREATE INDEX idx_setup_mistakes_status ON public.setup_mistakes(status);
CREATE INDEX idx_setup_mistakes_created_by ON public.setup_mistakes(created_by);

ALTER TABLE public.setup_mistakes ENABLE ROW LEVEL SECURITY;

-- View: any team member of the owning team, or workspace owner/admin
...
-- Insert: any authenticated team member; created_by must match auth user
...
-- Update (resolve): anyone on the team
...
-- Delete: workspace owner/admin or original creator
