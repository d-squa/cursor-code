-- Repair schema drift: PostgREST PGRST205 "Could not find the table 'public.qc_tracking'"
-- when migration 20260330124205 is recorded but the table was dropped or never created.

DO $$
BEGIN
  IF to_regclass('public.qc_tracking') IS NULL THEN
  BEGIN
  ...
  CREATE INDEX IF NOT EXISTS idx_qc_tracking_campaign ON public.qc_tracking(campaign_id);
  CREATE INDEX IF NOT EXISTS idx_qc_tracking_state ON public.qc_tracking(current_state);
  CREATE INDEX IF NOT EXISTS idx_qc_transitions_tracking ON public.qc_state_transitions(qc_tracking_id);
  CREATE INDEX IF NOT EXISTS idx_qc_transitions_campaign ON public.qc_state_transitions(campaign_id);
