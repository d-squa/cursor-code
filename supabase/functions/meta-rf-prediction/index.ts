...
serve(async (req) => {
  ...
  try {
    ...
    const maskedParams = new URLSearchParams(predictionParams).toString()
    ...
    if (!createResponse.ok) {
      ...
      try {
        ...
        if (errorData.error?.code === 100 && errorData.error?.message?.includes('buying_type')) {
          throw new Error('R&F_NOT_AVAILABLE: This ad account is not eligible for Reach & Frequency campaigns. The account must have RESERVED buying_type access.');
        }
        
        if (errorData.error?.code === 200) {
          throw new Error('PERMISSION_ERROR: Meta access token does not have required permissions. Need ads_management and business_management scopes for R&F predictions.');
        }
      } catch (e) {
        if (e instanceof Error && (e.message.startsWith('R&F_') || e.message.startsWith('INVALID_') || e.message.startsWith('PERMISSION_'))) {
          throw e;
        }
      }
    ...
    while (attempts < 10) {
      ...
      if (statusResponse.ok) {
        ...
        if (predictionResult.status === 1) { // 1 = ready
          break;
        }
      }
    ...
    const cpm = closestPoint.impression > 0 
      ? (closestPoint.budget / closestPoint.impression) * 1000 
      : 0;
    ...
    return new Response(
      JSON.stringify({ 
        error: error.message,
        details: 'Reach & Frequency predictions require: (1) Ad account with RESERVED buying_type access, (2) Access token with ads_management + business_management scopes, (3) App in Live mode'
      }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
