...
const META_VALID_OPTIMIZATION_GOALS: Record<string, string[]> = {
  ...
  OUTCOME_APP_PROMOTION: ["APP_INSTALLS", "OFFSITE_CONVERSIONS", "VALUE", "LINK_CLICKS"],
    ...
    for (const phase of phases) {
      try {
        ...
        for (const [strategyName, strategyKeywords] of strategiesToProcess) {
          ...
          if (advertisingChannelType === "PERFORMANCE_MAX") {
            console.log(`ℹ️ Performance Max campaign — building asset groups (no traditional ad groups)`);
            results.push({
              platform: "Google Ads",
              market: market.name,
              phase: phase.name,
