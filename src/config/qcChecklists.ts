...
const META_CAMPAIGN: QCChecklistItem[] = [
  { key: 'naming_taxonomy', label: 'Campaign naming follows taxonomy', description: 'Follow Meta taxonomy guide format' },
  { key: 'bo_number', label: 'BO number is added', description: 'Enter in campaign label/tags or campaign name suffix' },
  { key: 'special_ad_category', label: 'Special Ad Category is correctly set', description: 'If applicable (credit, housing, employment)' },
  { key: 'objective', label: 'Objective is correctly selected', description: 'Lead Gen / Awareness / Traffic / Conversions' },
  ...
  { key: 'advantage_plus', label: 'Advantage+ Campaign Setup', description: 'Confirm if used and settings are correct' },
  { key: 'spending_limit', label: 'Campaign spending limit added', description: 'Inline with BO amount' },
  { key: 'grasp_warnings', label: 'Grasp warnings green or comment provided', description: 'Check Grasp tool; comment if red' },
];
...
const META_ADSET: QCChecklistItem[] = [
  ...
  { key: 'facebook_page', label: 'Correct Facebook Page linked', description: 'For accurate attribution and reporting' },
  { key: 'optimization_goal', label: 'Optimization goal matches campaign objective', description: 'Verify conversion goal (Website Lead / Instant Forms)' },
  { key: 'performance_goal', label: 'Performance goal aligns with bid strategy', description: 'Confirm alignment' },
  { key: 'pixel_event', label: 'Pixel and conversion event configured', description: 'Ensure pixel and event are properly set' },
  { key: 'schedule', label: 'Schedule aligns with campaign timing', description: 'Verify date/time and dayparting if applicable' },
  ...
  { key: 'location', label: 'Location targeting accurate', description: 'Country/City/Radius; exclude non-target countries' },
  ...
  { key: 'retargeting_exclusion', label: 'Retargeting audience excluded from prospecting', description: 'If using retargeting, exclude from prospecting ad sets' },
  { key: 'grasp_warnings', label: 'Grasp warnings green or comment provided', description: 'Check Grasp tool; comment if red' },
];
...
const META_AD: QCChecklistItem[] = [
  ...
  { key: 'pages_selected', label: 'Correct Facebook/Instagram pages selected', description: 'Ensure proper attribution and branding' },
  ...
  { key: 'creative_uploaded', label: 'Correct creative (image/video) uploaded', description: 'Final version, correctly formatted' },
  ...
  { key: 'destination_link', label: 'Instant form or website link configured', description: 'Check form or destination link' },
  { key: 'language_match', label: 'Language matches targeting and creative', description: 'Ensure language consistency' },
  { key: 'ad_previews', label: 'Ad previews reviewed on all placements', description: 'Use preview tool to confirm layout' },
  { key: 'advantage_enhancements', label: 'All Advantage+ creative enhancements OFF', description: 'Except Relevant Comments' },
  { key: 'text_asset_fit', label: 'Text & asset fit across placements', description: 'Adapts well across Feed, Stories, Reels' },
...
const TIKTOK_CAMPAIGN: QCChecklistItem[] = [
  ...
  { key: 'objective', label: 'Objective & conversion goal correctly selected', description: 'Traffic, Conversions, Awareness; if Conversions, confirm TikTok event' },
  { key: 'campaign_setup', label: 'Confirm manual vs Smart+ campaign setup', description: 'As per strategy' },
  { key: 'budget_type', label: 'Budget type and amount set', description: 'Daily vs Lifetime; pacing standard vs accelerated' },
  ...
  { key: 'grasp_warnings', label: 'Grasp warnings green or comment provided', description: 'Check Grasp tool; comment if red' },
  { key: 'account_funded', label: 'Account funded before launch', description: 'Remind planners to add budget based on campaign budget' },
];

const TIKTOK_ADSET: QCChecklistItem[] = [
  { key: 'naming_taxonomy', label: 'Ad group naming follows taxonomy', description: 'PRO_UAE_MIXEDPLACEMENT_Interest format' },
  { key: 'optimization', label: 'Optimization location, goal, and event checked', description: 'Match campaign objective' },
  { key: 'placement', label: 'Manual placement selected', description: 'Remove Pangle placement' },
  { key: 'audience', label: 'Audience targeting accurate', description: 'Interest, Lookalike, Custom; demographics correct' },
...
const TIKTOK_AD: QCChecklistItem[] = [
  ...
  { key: 'creative_specs', label: 'Creative specs meet TikTok requirements', description: 'Duration, aspect ratio, resolution' },
  ...
  { key: 'caption_text', label: 'Caption text, hashtags, mentions match approved copy', description: 'Error-free and aligned' },
  { key: 'cta_button', label: 'CTA button appropriate for objective', description: 'Choose correctly' },
  { key: 'interactive_addons', label: 'Interactive add-ons added if required', description: 'Display Card, Countdown Sticker' },
...
const LINKEDIN_CAMPAIGN: QCChecklistItem[] = [
  ...
  { key: 'objective', label: 'Objective & conversion goal correctly selected', description: 'Website Visits, Lead Gen Form Opens, Video Views' },
  ...
  { key: 'access_permissions', label: 'Ad account access and permissions confirmed', description: 'Proper access levels' },
  { key: 'grasp_warnings', label: 'Grasp warnings green or comment provided', description: 'Check Grasp tool; comment if red' },
];
...
const LINKEDIN_ADSET: QCChecklistItem[] = [
  ...
  { key: 'audience_filters', label: 'Audience targeting filters configured', description: 'Location, Company, Job Title, Function, Seniority' },
  { key: 'matched_audiences', label: 'Matched Audiences added only when required', description: 'Retargeting/lookalike' },
  { key: 'audience_expansion', label: 'Audience expansion toggled on only when required', description: 'As per strategy' },
  { key: 'bid_budget', label: 'Bid type and budget correctly configured', description: 'Automated vs Maximum Delivery' },
  ...
  { key: 'remove_audience_network', label: 'Remove Audience Network from placements', description: 'Unless specifically required' },
...
const LINKEDIN_AD: QCChecklistItem[] = [
  ...
  { key: 'creative_specs', label: 'Creative asset meets LinkedIn specs', description: 'Dimensions, format, video length' },
  { key: 'ad_copy', label: 'Ad copy, headline, description match approved text', description: 'Error-free and aligned' },
  ...
  { key: 'dynamic_creative', label: 'Dynamic creative/templates toggled only when required', description: 'As per strategy' },
  { key: 'grasp_warnings', label: 'Grasp warnings green or comment provided', description: 'Check Grasp tool; comment if red' },
];
...
const GOOGLE_CAMPAIGN: QCChecklistItem[] = [
  ...
  { key: 'goal_type', label: 'Goal & campaign type correct', description: 'Sales, Leads, Traffic, Brand Awareness; Search, Display, PMax, Shopping, Video' },
  { key: 'budget_bidding', label: 'Budget & bidding reviewed', description: 'Daily/shared budget; Max Conversions, Target CPA/ROAS, Manual CPC' },
  { key: 'dates', label: 'Start and end dates correct', description: 'Match media plan' },
  { key: 'automated_features', label: 'Automated features configured', description: 'Smart Bidding, PMax asset groups, or manual settings' },
  { key: 'policy_compliance', label: 'Policy compliance & ad status checked', description: 'Review Policy Center for disapprovals' },
  { key: 'recommendations', label: 'Recommendations tab reviewed', description: 'Scan for critical/high-impact suggestions' },
...
const GOOGLE_ADSET: QCChecklistItem[] = [
  { key: 'naming_taxonomy', label: 'Ad group naming follows taxonomy', description: 'Follow Google ad group naming guide' },
  { key: 'conversion_action', label: 'Conversion action optimization', description: 'Verify optimization for correct conversion (Lead, Purchase)' },
  { key: 'parent_campaign', label: 'Parent campaign alignment', description: 'Under intended campaign type' },
  { key: 'bid_settings', label: 'Bidding & bid settings checked', description: 'Default max CPC or portfolio bid strategy' },
  { key: 'keywords_match', label: 'Keywords & match types correct', description: 'Exact, phrase, broad align with keyword theme' },
  { key: 'negative_keywords', label: 'Negative keywords applied', description: 'Shared and ad-group-level negatives block irrelevant terms' },
  { key: 'audience', label: 'Audience targeting confirmed', description: 'In-market, affinity, custom intent, remarketing lists' },
  { key: 'ad_rotation', label: 'Ad rotation & optimization reviewed', description: '"Optimize for best performing ads" vs "Rotate evenly"' },
  { key: 'device_adjustments', label: 'Device bid adjustments checked', description: 'Mobile/desktop/tablet modifiers' },
  { key: 'ad_schedule', label: 'Ad schedule overrides verified', description: 'If dayparting, hours/days align with plan' },
...
const GOOGLE_AD: QCChecklistItem[] = [
  ...
  { key: 'ad_format', label: 'Ad format & type matches campaign', description: 'RSA/ETA for Search; Image/HTML5 for Display; Video for YouTube' },
  { key: 'headlines_count', label: 'Headlines & descriptions count', description: '≥3 headlines & ≥2 descriptions per RSA' },
  { key: 'character_limits', label: 'Character limits respected', description: 'Headlines ≤30 chars; Descriptions ≤90 chars' },
  { key: 'headline_relevance', label: 'Headline relevance (Search)', description: 'Includes primary keyword in at least one headline' },
  ...
  { key: 'ad_preview', label: 'Ad preview QA performed', description: 'Check formatting on all devices & partner networks' },
  ...
  { key: 'asset_specs', label: 'Asset specs meet requirements', description: 'Images/videos meet size, format, file-size requirements' },
  { key: 'pmax_assets', label: 'Performance Max asset group', description: 'Contains required assets (logos, headlines, descriptions, images, videos)' },
  { key: 'demand_gen_video_off', label: 'Demand Gen: video enhancements OFF', description: 'For Demand Gen campaigns' },
  { key: 'demand_gen_video_format', label: 'Demand Gen: video format matches placement', description: '16:9 for in-feed; 9:16 for Shorts' },
  { key: 'pmax_video_off', label: 'PMax: Video Enhancement OFF', description: 'Under campaign settings > Asset optimization' },
  { key: 'pmax_url_exclusions', label: 'PMax: URL exclusions applied', description: 'For Text Customizations and URL Expansion' },
...
const X_CAMPAIGN: QCChecklistItem[] = [
  ...
  { key: 'objective', label: 'Objective & conversion goal correctly selected', description: 'Website Clicks, Conversions, App Installs, Awareness' },
...
const X_ADSET: QCChecklistItem[] = [
  ...
  { key: 'premium_subscribers', label: 'Premium subscribers targeted only when required', description: 'As per strategy' },
  { key: 'custom_audiences', label: 'Custom audiences added only when required', description: 'As per strategy' },
  { key: 'targeting_features', label: 'Targeting features configured properly', description: 'Keywords, Follower Lookalikes, Interests, Movies & TV Shows, Conversation Topics' },
  { key: 'retargeting', label: 'Retargeting features configured properly', description: 'Lists added correctly' },
  { key: 'optimized_targeting', label: 'Optimized Targeting toggled On only when part of strategy', description: 'As per strategy' },
  { key: 'exclusions', label: 'Exclusions applied', description: 'On ad account level (pre-configured for Events, VAD AO & Corporate)' },
  { key: 'grasp_warnings', label: 'Grasp warnings green or comment provided', description: 'Check Grasp tool; comment if red' },
];
...
const SNAPCHAT_AD: QCChecklistItem[] = [
  ...
  { key: 'creative_specs', label: 'Creative asset meets Snapchat specs', description: 'Dimensions, format, video length' },
...
export const CROSS_PLATFORM_CHECKLIST: QCChecklistItem[] = [
  // Pre-Launch
  { key: 'xp_budget_plan', label: 'Budget in BO and Plan checked', description: 'Verify against media plan', category: 'PRE-LAUNCH' },
  { key: 'xp_preview_links', label: 'Permalinks / Preview links checked and shared', description: 'Ad preview and functionality; share with Comms Team', category: 'PRE-LAUNCH' },
  { key: 'xp_landing_tags', label: 'Landing page checked for tags', description: 'Ensure tags are working before launch', category: 'PRE-LAUNCH' },
  { key: 'xp_correct_pixels', label: 'Correct pixels/tags used', description: 'Verify correct pixels for each platform', category: 'PRE-LAUNCH' },
  { key: 'xp_detailed_qc', label: 'Detailed QC performed before live', description: 'Use checklist before launch', category: 'PRE-LAUNCH' },
  // Tracking
  { key: 'xp_utm_tested', label: 'UTM parameters tested and correct', description: 'Ensure UTMs are added and functional', category: 'TRACKING' },
  { key: 'xp_conversion_tracking', label: 'Conversion tracking installed and verified', description: 'Events configured correctly', category: 'TRACKING' },
  { key: 'xp_view_tags', label: 'View tags added if required', description: 'Check with planning team', category: 'TRACKING' },
  { key: 'xp_auto_tagging', label: 'Auto-tagging enabled (where applicable)', description: 'For Amplitude and other analytics', category: 'TRACKING' },
  // Account
  { key: 'xp_account_naming', label: 'Ad account naming follows convention', description: 'Publicis naming convention', category: 'ACCOUNT' },
  { key: 'xp_user_access', label: 'User access rights reviewed', description: 'Clients (view/read-only), planners (standard), Performics (admin)', category: 'ACCOUNT' },
  { key: 'xp_billing', label: 'Billing profile correct', description: 'Linked properly on MCC level', category: 'ACCOUNT' },
  { key: 'xp_dataroma', label: 'New accounts linked to Dataroma', description: 'Send to Ghadeer', category: 'ACCOUNT' },
  { key: 'xp_grasp_label', label: 'New accounts labeled on Grasp', description: 'Send to manager', category: 'ACCOUNT' },
  { key: 'xp_country_exclusions', label: 'Country exclusions applied', description: 'Exclude Austria, France, Italy, Spain, Türkiye, United Kingdom if not targeting', category: 'ACCOUNT' },
  // Post-Launch
  { key: 'xp_post_launch_check', label: 'Campaigns checked after live', description: 'Check for rejected ads, disapprovals, etc.', category: 'POST-LAUNCH' },
  { key: 'xp_grasp_active', label: 'Grasp is on during execution', description: 'Ensure Grasp tool is active', category: 'POST-LAUNCH' },
  { key: 'xp_performance_review', label: 'Performance reviewed biweekly', description: 'Inform planners of recommendations', category: 'POST-LAUNCH' },
  { key: 'xp_budget_pacing', label: 'Budget pacing monitored', description: 'Check at least twice a week', category: 'POST-LAUNCH' },
  { key: 'xp_daily_budget', label: 'Campaign start with correct daily budget', description: 'Not with small daily budget', category: 'POST-LAUNCH' },
  { key: 'xp_end_date', label: 'End date added to all campaigns', description: 'Including account budget', category: 'POST-LAUNCH' },
  // Documentation
  { key: 'xp_email_thread', label: 'Important details in email thread', description: 'Keep communication documented', category: 'DOCUMENTATION' },
  { key: 'xp_group_chats', label: 'Group chats or emails for discussions', description: 'Not informal channels', category: 'DOCUMENTATION' },
  { key: 'xp_campaign_tracker', label: 'Campaign tracker updated', description: 'Accurately and on time', category: 'DOCUMENTATION' },
  { key: 'xp_bo_documented', label: 'BO number indicated in campaign tags/name', description: 'For internal tracking', category: 'DOCUMENTATION' },
];
