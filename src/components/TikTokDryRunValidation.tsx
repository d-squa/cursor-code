...
const DIMENSIONS = [
  { id: "objective_optgoal", label: "Objective × OptGoal", description: "All TikTok objectives with optimization goals" },
  { id: "bid_budget", label: "Bid Strategy & Budget", description: "Lowest Cost, Cost Cap + daily/lifetime" },
  { id: "placements", label: "Placements", description: "Automatic vs manual placement configs" },
  { id: "targeting", label: "Targeting", description: "Age/gender targeting variations" },
  { id: "conversion_events", label: "Conversion Events", description: "Pixel events (requires pixel ID)" },
];
...
export default function TikTokDryRunValidation() {
  ...
  const runValidation = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("tiktok-dry-run-validation", {
        body: {
      ...
      console.error("TikTok dry-run error:", err);
      toast.error(err.message || "Validation failed");
    } finally {
      setRunning(false);
    }
  }, [selectedDimensions, pixelId, advertiserId, countryCode]);
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            by creating <code className="bg-muted px-1 rounded text-xs">DISABLED</code> entities
            and immediately deleting them. No active campaigns are created.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-1">
              <Label className="text-xs">Pixel ID (optional, for conversion tests)</Label>
              <Input
                value={pixelId}
            ...
            <div className="space-y-1">
              <Label className="text-xs">Advertiser ID override (optional)</Label>
              <Input
                value={advertiserId}
        ...
        <>
                  ...
                  {Object.entries(response.errorGroups).map(([key, group]) => (
                    <Collapsible key={key} open={expandedGroups.has(key)}>
                      ...
                      <CollapsibleContent className="mt-1 p-3 bg-muted/30 rounded border text-xs">
                        ...
                        {group.error?.request_id && (
                          <p className="text-muted-foreground mb-2">Request ID: {group.error.request_id}</p>
                        )}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Advertiser: {response.summary.advertiserId} • Country: {response.summary.country} • {new Date(response.summary.testedAt).toLocaleString()}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ScrollArea className="h-[600px]">
                <div className="space-y-1">
                  {response.results.map((result) => {
                    const passed = result.campaignValidation.success && result.adGroupValidation.success;
                      ...
                      <div key={result.testId} className="border rounded">
                        ...
                        >
                          ...
                          <span className="text-xs truncate flex-1">
                            ...
                            {result.bidStrategy !== "BID_TYPE_NO_BID" && result.bidStrategy !== "Lowest Cost (Auto)" && (
                              <span className="text-muted-foreground"> • {result.bidStrategy}</span>
                            )}
                            {result.placementConfig !== "Automatic" && result.placementConfig !== "Default for objective" && (
                        ...
                        {isExpanded && (
                          <div className="px-3 pb-3 pt-1 text-xs border-t bg-muted/20 space-y-2">
                            ...
                            {!result.adGroupValidation.success && result.adGroupValidation.error && (
                              <div className="bg-destructive/10 p-2 rounded">
                                <p className="font-medium text-destructive">Ad Group Error:</p>
                                <pre className="whitespace-pre-wrap mt-1">
                                  {JSON.stringify(result.adGroupValidation.error, null, 2)}
                                </pre>
                              </div>
                            )}
                          </div>
