...
-- Trigger for automatic profile creation (drop first for idempotency)
...
DROP TRIGGER IF EXISTS update_connected_platforms_updated_at ON public.connected_platforms;
...
CREATE INDEX IF NOT EXISTS idx_connected_platforms_platform_type ON public.connected_platforms(platform_type);
CREATE INDEX IF NOT EXISTS idx_platform_accounts_connected_platform_id ON public.platform_accounts(connected_platform_id);
...
DO $$ BEGIN
  ALTER TABLE public.campaign_insights ADD CONSTRAINT campaign_insights_campaign_id_platform_key UNIQUE(campaign_id, platform);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DROP POLICY IF EXISTS "Users can view their own campaign insights" ON public.campaign_insights;
...
DROP POLICY IF EXISTS "Service role can manage all insights" ON public.campaign_insights;
...
CREATE INDEX IF NOT EXISTS idx_campaign_insights_fetched_at ON public.campaign_insights(fetched_at);

DROP TRIGGER IF EXISTS update_campaign_insights_updated_at ON public.campaign_insights;
...
DROP POLICY IF EXISTS "Team owners and admins can view invitations" ON public.invitations;
...
DROP POLICY IF EXISTS "Creators and admins can update invitations" ON public.invitations;
...
DROP POLICY IF EXISTS "Creators and admins can delete invitations" ON public.invitations;
...
ON public.user_roles
FOR SELECT
USING (can_view_roles_in_team(auth.uid(), team_id));
...
DROP POLICY IF EXISTS "Users can view modification requests for their campaigns" ON public.modification_requests;
...
DROP POLICY IF EXISTS "Campaign owners and assignees can update modification requests" ON public.modification_requests;
...
DROP POLICY IF EXISTS "Users can view history for their campaigns" ON public.campaign_change_history;
...
DROP POLICY IF EXISTS "Users can create history entries" ON public.campaign_change_history;
CREATE POLICY "Users can create history entries"
ON public.campaign_change_history FOR INSERT
WITH CHECK (user_id = auth.uid());

DROP TRIGGER IF EXISTS update_modification_requests_updated_at ON public.modification_requests;
...
DROP POLICY IF EXISTS "Team members can view team campaigns" ON public.campaigns;
...
DROP POLICY IF EXISTS "Team members can delete team campaigns" ON public.campaigns;
...
USING (
  EXISTS (
    ...
    JOIN user_roles ur_target ON ur_viewer.team_id = ur_target.team_id
    WHERE ur_viewer.user_id = auth.uid()
      AND ur_viewer.role = ANY (ARRAY['owner'::app_role, 'admin'::app_role])
...
DO $$ BEGIN
  ALTER TABLE public.meta_ad_accounts ADD CONSTRAINT meta_ad_accounts_user_account_key UNIQUE(user_id, account_id);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DROP POLICY IF EXISTS "Users can view their own ad accounts" ON public.meta_ad_accounts;
...
DROP POLICY IF EXISTS "Users can update their own ad accounts" ON public.meta_ad_accounts;
...
DO $$ BEGIN
  ALTER TABLE public.meta_product_sets ADD CONSTRAINT meta_product_sets_user_product_set_key UNIQUE(user_id, product_set_id);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DROP POLICY IF EXISTS "Users can view their own product sets" ON public.meta_product_sets;
CREATE POLICY "Users can view their own product sets"
  ON public.meta_product_sets FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own product sets" ON public.meta_product_sets;
CREATE POLICY "Users can insert their own product sets"
  ON public.meta_product_sets FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own product sets" ON public.meta_product_sets;
CREATE POLICY "Users can update their own product sets"
  ON public.meta_product_sets FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own product sets" ON public.meta_product_sets;
...
ALTER TABLE public.campaigns 
ADD COLUMN IF NOT EXISTS bo_number TEXT;

DO $$ BEGIN
  ALTER TABLE public.campaigns ADD CONSTRAINT campaigns_bo_number_key UNIQUE (bo_number);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DROP VIEW IF EXISTS connected_platforms_safe;
...
DO $$ BEGIN
  ALTER TABLE public.meta_pages ADD CONSTRAINT meta_pages_user_page_key UNIQUE(user_id, page_id);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DROP POLICY IF EXISTS "Users can view their own pages" ON public.meta_pages;
CREATE POLICY "Users can view their own pages"
ON public.meta_pages FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own pages" ON public.meta_pages;
CREATE POLICY "Users can insert their own pages"
ON public.meta_pages FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own pages" ON public.meta_pages;
CREATE POLICY "Users can update their own pages"
ON public.meta_pages FOR UPDATE
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own pages" ON public.meta_pages;
CREATE POLICY "Users can delete their own pages"
ON public.meta_pages FOR DELETE
USING (auth.uid() = user_id);

DROP VIEW IF EXISTS meta_pages_safe;
...
DROP POLICY IF EXISTS "Users can update their platform accounts" ON platform_accounts;
...
CREATE TABLE IF NOT EXISTS public.campaign_performance_benchmarks (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
...
DO $$ BEGIN
  ALTER TABLE public.campaign_performance_benchmarks 
    ADD CONSTRAINT campaign_performance_benchmarks_unique 
...
DROP POLICY IF EXISTS "Users can view their own benchmarks" ON public.campaign_performance_benchmarks;
CREATE POLICY "Users can view their own benchmarks"
ON public.campaign_performance_benchmarks
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own benchmarks" ON public.campaign_performance_benchmarks;
CREATE POLICY "Users can insert their own benchmarks"
ON public.campaign_performance_benchmarks
FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own benchmarks" ON public.campaign_performance_benchmarks;
CREATE POLICY "Users can update their own benchmarks"
ON public.campaign_performance_benchmarks
FOR UPDATE
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own benchmarks" ON public.campaign_performance_benchmarks;
CREATE POLICY "Users can delete their own benchmarks"
ON public.campaign_performance_benchmarks
FOR DELETE
USING (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_benchmarks_user_market_goal ON public.campaign_performance_benchmarks(user_id, market, optimization_goal);

DROP TRIGGER IF EXISTS update_benchmarks_updated_at ON public.campaign_performance_benchmarks;
...
CREATE INDEX IF NOT EXISTS idx_benchmarks_industry ON public.campaign_performance_benchmarks(industry);
...
DO $$ BEGIN
  ALTER TABLE public.team_clients ADD CONSTRAINT team_clients_team_client_key UNIQUE(team_id, client_id);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DROP POLICY IF EXISTS "Team members can view their team clients" ON public.team_clients;
...
DROP POLICY IF EXISTS "Admins and team owners can manage team clients" ON public.team_clients;
...
DROP POLICY IF EXISTS "Users can view their own clients or team clients" ON public.clients;
...
DO $$ BEGIN
  ALTER TABLE public.meta_instagram_accounts ADD CONSTRAINT meta_instagram_accounts_user_account_key UNIQUE(user_id, instagram_account_id);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DROP POLICY IF EXISTS "Users can view their own instagram accounts" ON public.meta_instagram_accounts;
CREATE POLICY "Users can view their own instagram accounts"
ON public.meta_instagram_accounts FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own instagram accounts" ON public.meta_instagram_accounts;
CREATE POLICY "Users can insert their own instagram accounts"
ON public.meta_instagram_accounts FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own instagram accounts" ON public.meta_instagram_accounts;
CREATE POLICY "Users can update their own instagram accounts"
ON public.meta_instagram_accounts FOR UPDATE
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own instagram accounts" ON public.meta_instagram_accounts;
CREATE POLICY "Users can delete their own instagram accounts"
ON public.meta_instagram_accounts FOR DELETE
USING (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "Users can view their own conversion events" ON public.meta_conversion_events;
CREATE POLICY "Users can view their own conversion events"
ON public.meta_conversion_events FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own conversion events" ON public.meta_conversion_events;
CREATE POLICY "Users can insert their own conversion events"
ON public.meta_conversion_events FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own conversion events" ON public.meta_conversion_events;
CREATE POLICY "Users can update their own conversion events"
ON public.meta_conversion_events FOR UPDATE
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own conversion events" ON public.meta_conversion_events;
CREATE POLICY "Users can delete their own conversion events"
ON public.meta_conversion_events FOR DELETE
USING (auth.uid() = user_id);
